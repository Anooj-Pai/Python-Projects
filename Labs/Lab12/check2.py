...
class MyApp(object):
    ...
    def draw_lines(self, centerx, centery, length):
        ...
        #base condition, checking if length of line is greater than 1 
        if length > 1:
            #count runs through each iteration of current plus sign and then
            #reduces the length of the sign to half it's current value.
            #if count == 0:
                ...
                self.draw_lines(centerx-length,centery,length/2)
                #count+=1
            # self.draw_lines(centerx-(length/2),centery+(length/2),length/2)
            # self.draw_lines(centerx,centery+(length/2))
            # self.draw_lines(centerx,centery-(length/2))
            # self.draw_lines(centerx+(length/2),centery)
