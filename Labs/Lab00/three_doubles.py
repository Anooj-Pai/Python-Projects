...
def has_three_double_space(word):
    """
    Returns True if the word contains three consecutive pairs of
    double letters and False otherwise.
    """
    for l in range(len(word) - 7):
        if word[l] == word[l + 1] and word[l + 3] == word[l + 4] and word[l + 6] == word[l + 7]:
            return True
    return False

def has_three_double_2space(word):
    """
    Returns True if the word contains three consecutive pairs of
    double letters and False otherwise.
    """
    for l in range(len(word) - 9):
        if word[l] == word[l + 1] and word[l + 4] == word[l + 5] and word[l + 8] == word[l + 9]:
            return True
    return False
...
for entry in word_file:
    ...
    if has_three_double_2space(entry):
        print(entry)
        count = count + 1
